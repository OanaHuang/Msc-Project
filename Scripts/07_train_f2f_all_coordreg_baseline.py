# Scripts/07_train_f2f_all_coordreg_baseline.py
...
# If running locally, fallback to script parent
...
IMAGE_SIZE = 224
BATCH_SIZE = 32          # Colab GPU can usually use 32
EPOCHS = 20
LR = 1e-4
VAL_RATIO = 0.2
NUM_WORKERS = 2          # If error, change to 0

# First test with 2000 or 5000.
# After it works, change to None.
MAX_SAMPLES = 30000
# MAX_SAMPLES = None
...
class PennActionAllKeypointsDataset(Dataset):
    def __init__(
        ...
        if max_samples is not None:
            ...
            print(f"Using first {len(self.image_paths)} samples")
        else:
    ...
    def _resolve_image_path(self, p):
        ...
        # If the npz stores Mac absolute path, keep only the part after Penn_Action
        if "Penn_Action" in p:
...
def main():
    ...
    model = ResNet18AllKeypointRegressor(
    ...
    print("\nStart training all-keypoint coordinate regression baseline")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_mse < best_val_mse:
            ...
            best_path = OUTPUT_DIR / "best_f2f_all_coordreg_resnet18.pth"
            ...
            torch.save(
                {
                    ...
                    "batch_size": BATCH_SIZE,
                    "lr": LR,
                    "model_name": "ResNet18AllKeypointRegressor",
    ...
    last_path = OUTPUT_DIR / "last_f2f_all_coordreg_resnet18.pth"
    ...
    print(f"Best model saved in: {OUTPUT_DIR / 'best_f2f_all_coordreg_resnet18.pth'}")
