# Scripts/08_infer_video_f2f_all_coordreg_baseline.py
...
CKPT_PATH = (
    ...
    / "outputs"
    / "f2f_all_coordreg_v1"
    / "best_f2f_all_coordreg_resnet18.pth"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "f2f_all_coordreg_v1"
...
# Whether to draw ground truth
SHOW_GT = True

# Whether to draw keypoint index numbers
# Strongly recommended: False, otherwise the video becomes messy
SHOW_KEYPOINT_INDEX = False

# Whether to draw title text at the top
SHOW_TITLE = True

# Prediction visual style
PRED_POINT_RADIUS = 2
PRED_LINE_THICKNESS = 1

# Ground-truth visual style
GT_POINT_RADIUS = 3
GT_LINE_THICKNESS = 1
...
class ResNet18AllKeypointRegressor(nn.Module):
...
def draw_keypoints(frame_bgr, pred_kpts, gt_kpts=None, visibility=None):
    """
    Red dots/lines = predicted keypoints
    Green circles/lines = ground truth keypoints
    """
    img = frame_bgr.copy()
...
def main():
    ...
    if not CKPT_PATH.exists():
        raise FileNotFoundError(
            ...
            f"Please run Scripts/07_train_f2f_all_coordreg_baseline.py first."
        )
    ...
    model = ResNet18AllKeypointRegressor(num_keypoints=num_keypoints)
    ...
    output_video_path = VIDEO_DIR / f"{target_video_id}_all_keypoints_pred_smallvis.mp4"
    video_writer = None
    ...
    output_pred_path = PRED_DIR / f"{target_video_id}_all_keypoints_predictions_smallvis.npz"
    ...
    np.savez_compressed(
        ...
        visibility=vis_keypoints,
        video_mse=video_mse,
        checkpoint=str(CKPT_PATH),
        image_size=image_size,
        num_keypoints=num_keypoints,
        method="All-keypoint Frame-by-frame Coordinate Regression"
    )
    ...
    print("Saved prediction video to:")
