...
# These values must match Model 21 training.
NUM_STEPS = 2
BETA = 0.90
THRESHOLD = 1.0
MAX_SPIKES = 4
...
# Save prediction coordinates and confidence values.
SAVE_PREDICTION_NPZ = True

# Set to 1 or 3 for a quick test.
# Set to None to process all single-person test videos.
MAX_TEST_VIDEOS = 1
...
def extract_sample_id_from_text(
    ...
    """
    Extract and normalise an NTU RGB+D sample ID.
...
def build_file_index(
    ...
    if ignored:
        print(
            "  Ignored files without NTU ID: "
            f"{ignored}"
        )

    if duplicates:
        print(
            "  Duplicate sample IDs ignored: "
            f"{duplicates}"
        )
...
def decode_heatmaps(
    ...
    Returns:
        keypoints:
            [J, 2] coordinates in the model input image

        confidence:
            [J] raw heatmap peak values
    """
    if heatmaps.ndim != 4:
        raise ValueError(
...
def draw_legend(
    ...
    # Prediction: yellow lines and red points.
    cv2.line(
        output,
        (25, 64),
        (55, 64),
...
def main() -> None:
    ...
    print("Readout type:        Mean integer spike features")
    ...
    print(f"I-LIF beta:          {BETA}")
    print(f"I-LIF threshold:     {THRESHOLD}")
    print(f"Integer range:       0..{MAX_SPIKES}")
