...
LAST_MODEL_PATH = (
    OUTPUT_DIR
    / "last_Spiking_ResNet50_Heatmap_ImageNet_T4.pth"
)
...
# 优先复用普通 ResNet50 baseline 的视频划分
...
def load_or_create_video_split(video_ids):
    if BASELINE_SPLIT_JSON_PATH.exists():
        print(
            "Loading split from ResNet50 baseline:"
        )
        ...
        if missing_keys:
            raise KeyError(
                "Baseline split JSON is missing keys: "
                f"{missing_keys}"
            )
        ...
        print(
            "ResNet50 baseline split was not found."
        )
        print(
            "Creating a new split with the same seed."
        )
...
class SpikingBottleneck(nn.Module):
    ...
    def forward(
        ...
        current = self.conv3(spike2)
        current = self.bn3(current)
...
# ============================================================
# 9. Spiking ResNet50 Backbone
# ============================================================

class SpikingResNet50Backbone(nn.Module):
...
# ============================================================
# 10. Full Heatmap Model
# ============================================================

class SpikingResNet50Heatmap(nn.Module):
...
def load_imagenet_resnet50_weights(
    spiking_model,
):
    print(
        "\nLoading ImageNet1K V2 ResNet50 "
        "weights into the spiking backbone..."
    )

    ann_resnet = models.resnet50(
        weights=(
            models
            .ResNet50_Weights
            .IMAGENET1K_V2
    ...
    spiking_backbone.bn1.load_state_dict(
        ann_resnet.bn1.state_dict()
    )
    ...
    for layer_index, (
        ...
        for block_index, (
            ...
            spiking_block.bn1.load_state_dict(
                ann_block.bn1.state_dict()
            )
            ...
            spiking_block.bn2.load_state_dict(
                ann_block.bn2.state_dict()
            )

            spiking_block.conv3.load_state_dict(
                ann_block.conv3.state_dict()
            )

            spiking_block.bn3.load_state_dict(
                ann_block.bn3.state_dict()
            )
            ...
            if (
                ...
                raise RuntimeError(
                    "Downsample mismatch at "
                    f"layer{layer_index}, "
                    f"block{block_index}."
                )

    print(
        "ImageNet ResNet50 weights loaded successfully."
    )
...
def main():
    ...
    print(
        ...
        "with ImageNet1K V2 Initialization"
    )
    print("=" * 76)
